package com.maestrodev.maestro.plugin;
...
public class Manifest {
    ...
    public Manifest(InputStream is) {
        Reader r = null;
        try {
            r = new InputStreamReader(this.getClass().getClassLoader().getResourceAsStream("manifest.json"));
    ...
    private void validatePlugin(JsonElement plugin) {
        ...
        if (plugin.isJsonObject()) {
            ...
            String[] fields = { "name", "description", "author", "version", "type", "class", "dependencies" };
            for (String f : fields) {
